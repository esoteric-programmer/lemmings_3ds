...
//Wave signed long available in the top of the 32bit range
//Original adlib uses 10.10, we use 10.22
#define WAVE_BITS	10
#else
//Need some extra signed long at the top to have room for octaves and frequency multiplier
//We support to 8 times lower rate
//128 * 15 * 8 = 15350, 2^13.9, so need 14 signed long
#define WAVE_BITS	14
#endif
...
//Maximum amount of attenuation signed long
//Envelope goes to 511, 9 signed long
#if (DBOPL_WAVE == WAVE_TABLEMUL )
//Uses the value directly
#define ENV_BITS	( 9 )
#else
//Add 3 signed long here for more accuracy and would have to be shifted up either way
#define ENV_BITS	( 9 )
#endif
//Limits of the envelope with those signed long and when the envelope goes silent
#define ENV_MIN		0
...
//Check some ranges
#if ENV_EXTRA > 3
#error Too many envelope signed long
#endif
...
#define M(_X_) ((u8)( (_X_) * 2))
static const u8 FreqCreateTable[16] = {
...
static const u8 AttackSamplesTable[13] = {
	69, 55, 46, 40,
	35, 29, 23, 20,
	19, 15, 11, 10,
	9
...
static const u8 EnvelopeIncreaseTable[13] = {
	4,  5,  6,  7,
	8, 10, 12, 14,
	16, 20, 24, 28,
	32,
...
static s16 WaveTable[ 8 * 512 ];
//Distance into WaveTable the wave starts
static const u16 WaveBaseTable[8] = {
	0x000, 0x200, 0x200, 0x800,
	0xa00, 0xc00, 0x100, 0x400,

};
//Mask the counter with this
static const u16 WaveMaskTable[8] = {
	1023, 1023, 511, 511,
	1023, 1023, 512, 1023,
};

//Where to start the counter on at keyon
static const u16 WaveStartTable[8] = {
	512, 0, 0, 0,
	0, 512, 512, 256,
};
#endif
...
#if ( DBOPL_WAVE == WAVE_TABLEMUL )
static u16 MulTable[ 384 ];
#endif

static u8 KslTable[ 8 * 16 ];
static u8 TremoloTable[ TREMOLO_TABLE ];
//Start of a channel behind the chip struct start
static u16 ChanOffsetTable[32];
//Start of an operator behind the chip struct start
static u16 OpOffsetTable[64];
...
*/
static inline signed long MakeVolume( unsigned long wave, unsigned long volume ) {
	unsigned long total = wave + volume;
	unsigned long index = total & 0xff;
	unsigned long sig = ExpTable[ index ];
	unsigned long exp = total >> 8;
#if 0
...
static signed long DB_FASTCALL WaveForm0( unsigned long i, unsigned long volume ) {
	signed long neg = 0 - (( i >> 9) & 1);//Create ~0 or 0
	unsigned long wave = SinTable[i & 511];
	return (MakeVolume( wave, volume ) ^ neg) - neg;
}
static signed long DB_FASTCALL WaveForm1( unsigned long i, unsigned long volume ) {
	u32 wave = SinTable[i & 511];
	wave |= ( ( (i ^ 512 ) & 512) - 1) >> ( 32 - 12 );
	return MakeVolume( wave, volume );
}
static signed long DB_FASTCALL WaveForm2( unsigned long i, unsigned long volume ) {
	unsigned long wave = SinTable[i & 511];
	return MakeVolume( wave, volume );
}
static signed long DB_FASTCALL WaveForm3( unsigned long i, unsigned long volume ) {
	unsigned long wave = SinTable[i & 255];
	wave |= ( ( (i ^ 256 ) & 256) - 1) >> ( 32 - 12 );
	return MakeVolume( wave, volume );
}
static signed long DB_FASTCALL WaveForm4( unsigned long i, unsigned long volume ) {
	//Twice as fast
	i <<= 1;
	signed long neg = 0 - (( i >> 9) & 1);//Create ~0 or 0
	unsigned long wave = SinTable[i & 511];
...
static signed long DB_FASTCALL WaveForm5( unsigned long i, unsigned long volume ) {
	//Twice as fast
	i <<= 1;
	unsigned long wave = SinTable[i & 511];
	wave |= ( ( (i ^ 512 ) & 512) - 1) >> ( 32 - 12 );
...
static signed long DB_FASTCALL WaveForm6( unsigned long i, unsigned long volume ) {
	signed long neg = 0 - (( i >> 9) & 1);//Create ~0 or 0
	return (MakeVolume( 0, volume ) ^ neg) - neg;
}
static signed long DB_FASTCALL WaveForm7( unsigned long i, unsigned long volume ) {
	//Negative is reversed here
	signed long neg = (( i >> 9) & 1) - 1;
	unsigned long wave = (i << 3);
...
inline void Operator::UpdateAttenuation( ) {
	u8 kslBase = (u8)((chanData >> SHIFT_KSLBASE) & 0xff);
	u32 tl = reg40 & 0x3f;
	u8 kslShift = KslShiftTable[ reg40 >> 6 ];
	//Make sure the attenuation goes to the right signed long
	totalLevel = tl << ( ENV_BITS - 7 );	//Total level goes 2 signed long below max
	totalLevel += ( kslBase << ENV_EXTRA ) >> kslShift;
}

void Operator::UpdateFrequency(  ) {
	u32 freq = chanData & (( 1 << 10 ) - 1);
	u32 block = (chanData >> 10) & 0xff;
...
void Operator::UpdateRates( const Chip* chip ) {
	...
	u8 newKsr = (u8)((chanData >> SHIFT_KEYCODE) & 0xff);
	if ( !( reg20 & MASK_KSR ) ) {
		newKsr >>= 2;
	}
	if ( ksr == newKsr )
...
inline s32 Operator::RateForward( u32 add ) {
	rateIndex += add;
	s32 ret = rateIndex >> RATE_SH;
	rateIndex = rateIndex & RATE_MASK;
	return ret;
}

template< Operator::State yes>
signed long Operator::TemplateVolume(  ) {
	s32 vol = volume;
	s32 change;
	switch ( yes ) {
	case OFF:
...
inline unsigned long Operator::ForwardVolume() {
...
void Operator::Write60( const Chip* chip, u8 val ) {
	u8 change = reg60 ^ val;
	reg60 = val;
	if ( change & 0x0f ) {
		UpdateDecay( chip );
...
inline void Operator::SetState( u8 s ) {
	state = s;
...
inline void Operator::Prepare( const Chip* chip )  {
	...
	if ( vibStrength >> chip->vibratoShift ) {
		s32 add = vibrato >> chip->vibratoShift;
...
inline signed long Operator::GetWave( unsigned long index, unsigned long vol ) {
#if ( DBOPL_WAVE == WAVE_HANDLER )
	...
	s32 wave = waveBase[ index & waveMask ];
	u32 total = ( wave & 0x7fff ) + vol << ( 3 - ENV_EXTRA );
	s32 sig = ExpTable[ total & 0xff ];
	u32 exp = total >> 8;
	s32 neg = wave >> 16;
...
signed long inline Operator::GetSample( signed long modulation ) {
	unsigned long vol = ForwardVolume();
	if ( ENV_SILENT( vol ) ) {
		//Simply forward the wave
		waveIndex += waveCurrent;
		return 0;
	} else {
		unsigned long index = ForwardWave();
		index += modulation;
...
void Channel::SetChanData( const Chip* chip, u32 data ) {
	u32 change = chanData ^ data;
	chanData = data;
	Op( 0 )->chanData = data;
	Op( 1 )->chanData = data;
...
void Channel::UpdateFrequency( const Chip* chip, u8 fourOp ) {
	//Extrace the frequency signed long
	u32 data = chanData & 0xffff;
	u32 kslBase = KslTable[ data >> 6 ];
	u32 keyCode = ( data & 0x1c00) >> 9;
	if ( chip->reg08 & 0x40 ) {
...
void Channel::WriteA0( const Chip* chip, u8 val ) {
	u8 fourOp = chip->reg104 & chip->opl3Active & fourMask;
	//Don't handle writes to silent fourop channels
	if ( fourOp > 0x80 )
		return;
	u32 change = (chanData ^ val ) & 0xff;
	if ( change ) {
		chanData ^= change;
...
void Channel::WriteB0( const Chip* chip, u8 val ) {
	u8 fourOp = chip->reg104 & chip->opl3Active & fourMask;
	//Don't handle writes to silent fourop channels
	if ( fourOp > 0x80 )
		return;
	unsigned long change = (chanData ^ ( val << 8 ) ) & 0x1f00;
	if ( change ) {
		chanData ^= change;
		UpdateFrequency( chip, fourOp );
	}
...
void Channel::WriteC0( const Chip* chip, u8 val ) {
	u8 change = val ^ regC0;
	if ( !change )
		return;
	regC0 = val;
	...
	if ( chip->opl3Active ) {
		...
		if ( (chip->reg104 & fourMask) & 0x3f ) {
			...
			u8 synth = ( (chan0->regC0 & 1) << 0 )| (( chan1->regC0 & 1) << 1 );
			switch ( synth ) {
			case 0:
...
inline void Channel::GeneratePercussion( Chip* chip, s32* output ) {
	Channel* chan = this;
	...
	u32 noiseBit = chip->ForwardNoise() & 0x1;
	u32 c2 = Op(2)->ForwardWave();
	u32 c5 = Op(5)->ForwardWave();
	u32 phaseBit = (((c2 & 0x88) ^ ((c2<<5) & 0x80)) | ((c5 ^ (c5<<2)) & 0x20)) ? 0x02 : 0x00;

	//Hi-Hat
	u32 hhVol = Op(2)->ForwardVolume();
	if ( !ENV_SILENT( hhVol ) ) {
		u32 hhIndex = (phaseBit<<8) | (0x34 << ( phaseBit ^ (noiseBit << 1 )));
		sample += Op(2)->GetWave( hhIndex, hhVol );
	}
	//Snare Drum
	u32 sdVol = Op(3)->ForwardVolume();
	if ( !ENV_SILENT( sdVol ) ) {
		u32 sdIndex = ( 0x100 + (c2 & 0x100) ) ^ ( noiseBit << 8 );
	...
	//Top-Cymbal
	u32 tcVol = Op(5)->ForwardVolume();
	if ( !ENV_SILENT( tcVol ) ) {
		u32 tcIndex = (1 + phaseBit) << 8;
...
Channel* Channel::BlockTemplate( Chip* chip, u32 samples, s32* output ) {
	switch( mode ) {
	case sm2AM:
	case sm3AM:
	...
	for ( unsigned long i = 0; i < samples; i++ ) {
		...
		if ( mode == sm2AM || mode == sm3AM ) {
			...
			sample = Op(1)->GetSample( out0 );
			signed long next = Op(2)->GetSample( 0 );
			sample += Op(3)->GetSample( next );
...
inline u32 Chip::ForwardNoise() {
	noiseCounter += noiseAdd;
	unsigned long count = noiseCounter >> LFO_SH;
...
inline u32 Chip::ForwardLFO( u32 samples ) {
	...
	u32 todo = LFO_MAX - lfoCounter;
	u32 count = (todo + lfoAdd - 1) / lfoAdd;
	if ( count > samples ) {
		count = samples;
...
void Chip::WriteReg( u32 reg, u8 val ) {
	unsigned long index;
	switch ( (reg & 0xf0) >> 4 ) {
	case 0x00 >> 4:
		if ( reg == 0x01 ) {
...
u32 Chip::WriteAddr( u32 port, u8 val ) {
	switch ( port & 3 ) {
	case 0:
		return val;
	case 2:
...
void Chip::GenerateBlock2( unsigned long total, s32* output ) {
	while ( total > 0 ) {
		u32 samples = ForwardLFO( total );
		memset(output, 0, sizeof(s32) * samples);
		int count = 0;
...
void Chip::GenerateBlock3( unsigned long total, s32* output  ) {
	while ( total > 0 ) {
		u32 samples = ForwardLFO( total );
		memset(output, 0, sizeof(s32) * samples *2);
		int count = 0;
...
//	double original = rate;
	...
	for ( int i = 0; i < 16; i++ ) {
		freqMul[i] = (u32)( 0.5 + freqScale * FreqCreateTable[ i ] );
	}
#else
	u32 freqScale = (u32)( 0.5 + scale * ( 1 << ( WAVE_SH - 1 - 10)));
	for ( int i = 0; i < 16; i++ ) {
	...
	for ( u8 i = 0; i < 76; i++ ) {
		...
		linearRates[i] = (u32)( scale * (EnvelopeIncreaseTable[ index ] << ( RATE_SH + ENV_EXTRA - shift - 3 )));
	}
	...
	for ( u8 i = 0; i < 62; i++ ) {
		...
		u32 bestDiff = 1 << 30;
		for( u32 passes = 0; passes < 16; passes ++ ) {
			s32 volume = ENV_MAX;
			s32 samples = 0;
			u32 count = 0;
			...
			if ( diff < 0 ) {
				...
				s32 mul = ((original - diff) << 12) / original;
...
void InitTables( void ) {
	...
	for ( int i = 0; i < 512; i++ ) {
		SinTable[i] = (s16)( 0.5 - log10( sin( (i + 0.5) * (PI / 512.0) ) ) / log10(2.0)*256 );
	}
#endif
#if ( DBOPL_WAVE == WAVE_TABLEMUL )
	...
	//Sine Wave Base
	for ( int i = 0; i < 512; i++ ) {
		WaveTable[ 0x0200 + i ] = (s16)(sin( (i + 0.5) * (PI / 512.0) ) * 4084);
		WaveTable[ 0x0000 + i ] = -WaveTable[ 0x200 + i ];
	}
	//Exponential wave
	for ( int i = 0; i < 256; i++ ) {
		WaveTable[ 0x700 + i ] = (s16)( 0.5 + ( pow(2.0, -1.0 + ( 255 - i * 8) * ( 1.0 /256 ) ) ) * 4085 );
	...
	//Sine Wave Base
	for ( int i = 0; i < 512; i++ ) {
		WaveTable[ 0x0200 + i ] = (s16)( 0.5 - log10( sin( (i + 0.5) * (PI / 512.0) ) ) / log10(2.0)*256 );
		WaveTable[ 0x0000 + i ] = ((s16)0x8000) | WaveTable[ 0x200 + i];
	}
	//Exponential wave
	for ( int i = 0; i < 256; i++ ) {
	...
	for ( u8 i = 0; i < TREMOLO_TABLE / 2; i++ ) {
		u8 val = i << ENV_EXTRA;
		TremoloTable[i] = val;
	...
	for ( unsigned long i = 0; i < 32; i++ ) {
		...
		//Add back the signed long for highest ones
		if ( i >= 16 )
			index += 9;
		unsigned long blah = reinterpret_cast<unsigned long>( &(chip->chan[ index ]) );
	...
	for ( unsigned long i = 0; i < 64; i++ ) {
		...
		unsigned long chNum = (i / 8) * 3 + (i % 8) % 3;
		//Make sure we use 16 and up for the 2nd range to match the chanoffset gap
		if ( chNum >= 12 )
			chNum += 16 - 12;
		unsigned long opNum = ( i % 8 ) / 3;
		DBOPL::Channel* chan = 0;
		unsigned long blah = reinterpret_cast<unsigned long>( &(chan->op[opNum]) );
		OpOffsetTable[i] = ChanOffsetTable[ chNum ] + blah;
	}
#if 0
	//Stupid checks if table's are correct
	for ( unsigned long i = 0; i < 18; i++ ) {
		u32 find = (u16)( &(chip->chan[ i ]) );
		for ( unsigned long c = 0; c < 32; c++ ) {
	...
	for ( unsigned long i = 0; i < 36; i++ ) {
		u32 find = (u16)( &(chip->chan[ i / 2 ].op[i % 2]) );
		for ( unsigned long c = 0; c < 64; c++ ) {
...
u32 Handler::WriteAddr( u32 port, u8 val ) {
	return chip.WriteAddr( port, val );

}
void Handler::WriteReg( u32 addr, u8 val ) {
