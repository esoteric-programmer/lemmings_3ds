...
struct LevelResult run_level(
	...
	main_data->high_perf_palette[7] = level->palette.vga[8];
	...
	while ((apt_result = aptMainLoop())) {
		...
		do {
			...
			if (fade_in) {
				...
				fade_out ++;
				if (fade_out > FADE_OUT_DOSFRAMES) {
					// exit
					next_frame = 0;
					break;
				}
			...
			if (count_lemmings(lemmings) + lemmings_left(level->info.lemmings) == 0 ||
					state.frames_left == 0) {
				if (!fade_out) {
					fade_out = 1;
				}
		...
		float fade = 1.0;
		if (fade_in) {
			fade = ((float)(FADE_IN_DOSFRAMES-fade_in)) / ((float)FADE_IN_DOSFRAMES);
		}else if (fade_out) {
			fade = ((float)(FADE_OUT_DOSFRAMES-fade_out)) / ((float)FADE_OUT_DOSFRAMES);
		}
		u32 level_palette_faded[16];
		memcpy(level_palette_faded,level->palette.vga,4*16);
...
int show_result(
	...
	draw_menu_text(BOTTOM_SCREEN_BACK,
			menu_data,
			0,
